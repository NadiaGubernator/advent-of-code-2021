...
draw_numbers.each do |draw_number|
  boards.each_with_index do |board, board_index|
    board.each_with_index do |line, line_index|
      ...
      elements = line.uniq
      return p "Bingo! #{board_sum(board) * draw_number.to_i}" if winning_boards.length == boards.length
      winning_boards[board_index] = draw_number if elements.length == 1 && elements.first == 'x'
    end
